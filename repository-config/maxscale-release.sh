...
production_repo_prefix=${production_repo_prefix:-"https://downloads.mariadb.com/MaxScale/"}
...
export rpm_repo_key=${production_repo_prefix}/MariaDB-MaxScale-GPG-KEY

cat old_key_versions | grep "^$version$"
if [ $? == 0 ]; then
	echo "old key!"
	export deb_repo_key="70E4618A8167EE24"
	export rpm_repo_key=${production_repo_prefix}/old_key.public
fi
...
" 2> /dev/null > $dir/$dest/${platform}_${platform_version}_$version.json
	done
done

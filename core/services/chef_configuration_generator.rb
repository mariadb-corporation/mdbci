...
module ChefConfigurationGenerator
  ...
  def self.install_product(name, config, logger, network_settings, machine_configurator, product,
                           need_rewrite, repos, product_version, repo_key, hard_specify, recipe_name)
    generate_role_file(name, config, product, repos, product_version, repo_key,  recipe_name, hard_specify).and_then do |role_file_path|
  ...
  def self.generate_role_file(name, config, product, repos, product_version, repo_key, recipe_name, hard_specify)
    ...
    ConfigurationGenerator
      .generate_product_config(repos, product, product_hash, box, nil, config.provider, hard_specify)
      .and_then do |configs|

...
class TerraformConfigurator
    ...
    TerraformService.resource_type(@config.provider).and_then do |resource_type|
      ...
      @attempts.times do |attempt|
        ...
        TerraformService.running_resources(@ui, @config.path).and_then do |running_resources|
          target_nodes = target_nodes.reject do |_node, resource|
            running_resources.include?(resource)
          end
        end
        ...
        target_nodes = TerraformService.nodes_to_resources(nodes_network_result.error,
                                                           resource_type)
      end
  ...
  def resources_to_bring(nodes, resource_type)
    TerraformService.nodes_to_resources(nodes, resource_type)
  end
  ...
  def configure_node(node, logger)
    ...
    @attempts.times do |attempt|
      ...
      node_network = SshUser.create_user(@machine_configurator, node, node_network, @config.path,
                                         logger)
      @network_settings.add_network_configuration(node, node_network)
      result = NetworkChecker.resources_available?(@machine_configurator, node_network,
                                                   logger).and_then do
        ChefConfigurationGenerator.configure_with_chef(node, logger,
                                                       @network_settings.node_settings(node), @config, @ui, @machine_configurator)
      end
  ...
  def retrieve_all_network_settings(nodes)
    all_nodes_network_settings = nodes.map do |node|
      ...
      network_settings = TerraformService.resource_network(node, @ui,
                                                           @config.path).and_then do |node_network|
        result = {

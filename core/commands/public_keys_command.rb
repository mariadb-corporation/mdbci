...
class PublicKeysCommand < BaseCommand
 ...
 mdbci public_keys --key location/keyfile.file --labels label config
  ...
  def configure_server_ssh_key(machine)
    ...
    SshCommands.execute_command_with_ssh(machine,
                                         'cat ~/.ssh/authorized_keys').and_then do |authorized_keys_content|
      unless authorized_keys_content.include? key_file_content
        return SshCommands.execute_command_with_ssh(machine,
                                                    "echo '#{key_file_content}' >> ~/.ssh/authorized_keys")
      end
    end
  end

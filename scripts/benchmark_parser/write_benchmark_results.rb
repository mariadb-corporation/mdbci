...
def write_to_performance_test_run(client, build_params)
  ...
  mdbci_template_content = if File.file?(build_params[MDBCI_TEMPLATE])
                             File.read(build_params[MDBCI_TEMPLATE])
                           else
                             ''
                           end
  # Submit entry
...
def write_to_product_parameters(client, build_params, test_run_id)
  ...
  return unless build_params[PRODUCT_UNDER_TEST] == PRODUCT_MAXSCALE
...
def write_to_benchmark_results(client, test_tool, benchmark_results, test_run_id)
  ...
  return unless test_tool == BENCHMARK_SYSBENCH
...
def validate_hash(hash)
  puts 'validate_hash'
  puts hash.keys
  return unless !hash.key?(BUILD_PARAMS) or !hash.key?(BENCHMARK_RESULTS)

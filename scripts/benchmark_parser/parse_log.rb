...
def extract_sysbench_results_raw(input_file)
  ...
  File.open(input_file, 'r') do |f|
    f.each_line do |line|
      ...
      next unless sysbench_block_found

      line = line.match(SYSBENCH_BLOCK_LINE_REGEX).captures[0] if line =~ SYSBENCH_BLOCK_LINE_REGEX
  ...
  raise 'Performance test failed before gathering SQL statistics' if sysbench_results_raw == ''
...
def get_test_code_commit
  ...
  return git_log.lines.first.match(commit_regex).captures[0] if git_log.lines.first =~ commit_regex

  'NOT FOUND'
end

def get_build_params_hash
  template_path = ENV['name'] ? "#{ENV.fetch('HOME', nil)}/mdbci/#{ENV['name']}.json" : 'NOT FOUND'
  cnf_path = if !$maxscale_config_file_path.nil? && File.exist?($maxscale_config_file_path)
               $maxscale_config_file_path
             else
               'NOT FOUND'
             end
  {
...
def write_hash_to_json(hash, output_file)
  File.write(output_file, hash.to_json)
end

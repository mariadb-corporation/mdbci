...
class LogParser
  attr_reader :cmake_flags
  attr_reader :maxscale_source
  attr_reader :target
  attr_reader :mariadb_version
  attr_reader :box
  ...
  def parse_ctest_log(log)
    ...
    log.each_line do |line|
      ...
      if @mariadb_version.nil?
        @mariadb_version = extract_value_from_str(line,
                                                  mariadb_version_regex)
      end
      ...
      if @maxscale_source.nil?
        @maxscale_source = extract_value_from_str(line,
                                                  maxscale_source_regex)
      end
  ...
  def parse_test_results(log)
    test_regex = %r{(\d+)/(\d+)\s+Test\s+#(\d+):[\s]+([^\s]+)\s+[\.\*]+([^\d]+)([\d\.]+)}

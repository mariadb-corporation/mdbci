...
class CTestParser
  ...
  def parse_ctest_log
    ...
    logs_dir_regex = %r{^Logs go to /home/vagrant/LOGS/(.+)$}
    ...
    $log.each_line do |line|
      maxscale_version_end_found = true if line =~ maxscale_version_end_regex
      if maxscale_version_start_found and !maxscale_version_end_found and !line.gsub(/\n*/,
                                                                                     '').empty?
        @maxscale_entity.push line.gsub(/\n*/, '')
      end
      maxscale_version_start_found = true if line =~ maxscale_version_start_regex
    ...
    if @ctest_executed
      ...
      @failed_ctest_info = { TESTS_COUNT => NOT_FOUND, FAILED_TESTS_COUNT => NOT_FOUND,
                             TESTS => [] }
    end
  end
  ...
  def find_tests_info(ctest_log)
    ...
    ctest_sublog = []
    ctest_log.each do |line|
      test_end_regex = %r{(\d+)/(\d+)\s+Test\s+#(\d+):[\s]+([^\s]+)\s+[\.\*]+([^\d]+)([\d\.]+)}
      ctest_sublog.push(line) unless FIRST_LINES_CTEST_TO_SKIP.include? line
      next unless line =~ test_end_regex
      ...
      next unless test_success != PASSED
  ...
  def get_test_code_commit
    ...
    Dir.chdir ENV.fetch(WORKSPACE, nil)
    git_log = `git log -1`
  ...
  def generate_hr_result(parsed_ctest_data)
    ...
    hr_tests.push ''
    maxscale_commit = @maxscale_commit || NOT_FOUND
    maxscale_source = @maxscale_source || NOT_FOUND
    cmake_flags = @cmake_flags || NOT_FOUND
    logs_dir = @logs_dir || NOT_FOUND
    ...
    hr_tests += generate_run_test_build_parameters_hr
  ...
  def generate_mr_result(parsed_ctest_data)
    ...
    unless @ctest_executed
      parsed_ctest_data = { ERROR => CTEST_NOT_EXECUTED_ERROR }.merge(parsed_ctest_data)
    end
    JSON.pretty_generate parsed_ctest_data
  end
  ...
  def save_result_to_file
    open($output_log_file_path, 'w') do |f|
      ...
      f.puts [BUILD_LOG_PARSING_RESULT,
              generate_hr_result(ctest_info)].join(NEW_LINE_JENKINS_FORMAT)
    end
  end

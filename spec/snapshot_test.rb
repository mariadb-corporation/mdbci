...
def cmd_generate(destination)
  "./mdbci --template #{PATH_TO_TEMPLATES}/#{destination}.json generate #{destination}"
end

def cmd_up(destination)
  "./mdbci up #{destination}"
end

def cmd_run_command(command, destination, node_name)
  "./mdbci sudo --command #{command} #{destination}/#{node_name}"
end

def cmd_setup_product_repo(product, product_version, destination, node_name)
  "./mdbci setup_repo --product #{product} --product-version #{product_version} #{destination}/#{node_name}"
end

def cmd_install_product(product, destination, node_name)
  "./mdbci install_product --product #{product} #{destination}/#{node_name}"
end

def cmd_snapshot_list(destination, node_name)
  "./mdbci snapshot --silent list --path-to-nodes #{destination} --node-name #{node_name}"
end

def cmd_snapshot_take(destination, node_name, snapshot_name)
  "./mdbci snapshot take --path-to-nodes #{destination} --node-name #{node_name} --snapshot-name #{snapshot_name}"
end

def cmd_snapshots_take(destination, snapshot_name)
  "./mdbci snapshot take --path-to-nodes #{destination} --snapshot-name #{snapshot_name}"
end

def cmd_snapshot_revert(destination, node_name, snapshot_name)
  "./mdbci snapshot revert --path-to-nodes #{destination} --node-name #{node_name} --snapshot-name #{snapshot_name}"
end

def cmd_snapshots_revert(destination, snapshot_name)
  "./mdbci snapshot revert --path-to-nodes #{destination} --snapshot-name #{snapshot_name}"
end

def cmd_snapshot_remove(destination, node_name, snapshot_name)
  "./mdbci snapshot remove --path-to-nodes #{destination} --node-name #{node_name} --snapshot-name #{snapshot_name}"
end

def generate_machines(destination)
  return unless execute_bash(cmd_generate(destination)) != 0

  raise "#{TEST_ERROR_TAG} #{CANNOT_GENERATE_CONFIG_ERROR} #{destination}"
end

def start_machines(destination)
  return unless execute_bash(cmd_up(destination)) != 0
...
def clear_network_configurations
  ...
  libvirt_networks.each_line do |network|
    execute_bash "virsh net-destroy #{network}" if network != 'default'
  end
  ...
  virtualbox_networks.each_line do |network|
    execute_bash "VBoxManage hostonlyif remove #{network}" if network != 'default'
  end
end
  ...
  describe "Individual nodes snapshot test in destination: #{destination}" do
    ...
    NODES_NAMES.each do |node_name|
      it "creating initial snapshot in #{destination} for #{node_name}" do
        execute_bash(cmd_snapshot_take(destination, node_name,
                                       SNAPSHOTS_NAMES[destination][:initial])).should eql 0
      end

      it 'checking that snapshot was created' do
        `#{cmd_snapshot_list(destination,
                             node_name)}`.should eql SNAPSHOT_LIST_OUTPUT[destination][:initial]
      end
      ...
      it "creating snapshot with mariadb 5.5 installed in #{destination} for #{node_name}" do
        execute_bash(cmd_snapshot_take(destination, node_name,
                                       SNAPSHOTS_NAMES[destination][:initial_mariadb])).should eql 0
      end

      it 'checking that snapshot was created' do
        `#{cmd_snapshot_list(destination,
                             node_name)}`.should eql SNAPSHOT_LIST_OUTPUT[destination][:initial_mariadb]
      end

      it "reverting #{node_name} to initial snapshot" do
        execute_bash(cmd_snapshot_revert(destination, node_name,
                                         SNAPSHOTS_NAMES[destination][:initial])).should eql 0
      end
      ...
      it 'removing initial snapshot' do
        execute_bash(cmd_snapshot_remove(destination, node_name,
                                         SNAPSHOTS_NAMES[destination][:initial])).should eql 0
      end
      ...
      if destination == DESTINATIONS[1]
        ...
        it "trying to remove last docker snapshot, but it's not going to happened (because snapshot is in use)" do
          execute_bash(cmd_snapshot_remove(destination, node_name,
                                           SNAPSHOTS_NAMES[destination][:initial_mariadb])).should_not eql 0
        end

      else # DESTINATION[1] = snapshot_test_docker
...
DESTINATIONS.each do |destination|
  ...
  describe "Buch of nodes snapshot test in destination: #{destination}" do
    ...
    it "creating initial snapshots in #{destination} for all nodes" do
      execute_bash(cmd_snapshots_take(destination,
                                      SNAPSHOTS_NAMES[destination][:initial])).should eql 0
    end
    ...
    it 'reverting all nodes to initial snapshots' do
      execute_bash(cmd_snapshots_revert(destination,
                                        SNAPSHOTS_NAMES[destination][:initial])).should eql 0
    end

...
describe 'Session' do
  ...
  it '#getBoxNameByPath should raise wrong node' do
    lambda {
      $session.boxes.getBoxNameByPath('spec/test_machine_configurations/6821_test_conf/node123')
    }.should(raise_error(
               RuntimeError, /Node .* is not found in .*/
             ))
  end

  it '#getBoxNameByPath should raise wrong path' do
    lambda {
      $session.boxes.getBoxNameByPath('WRONG_PATH')
    }.should(raise_error(RuntimeError,
                         'Path to generated nodes configurations is wrong'))
  end
end

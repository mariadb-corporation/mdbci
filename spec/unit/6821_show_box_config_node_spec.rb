...
describe 'Session' do
  DOCKER_CONF = ENV.fetch('mdbci_param_conf_docker', nil)
  ...
  it '#getBoxByGeneratedConfig raises boxes for generated configuration' do
    lambda {
      $session.boxes.getBoxByGeneratedConfig('WRONG')
    }.should raise_error 'Path to generated nodes configurations is wrong'
  end

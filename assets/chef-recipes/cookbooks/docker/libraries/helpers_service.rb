...
module DockerCookbook
  module DockerHelpers
    unless defined?(DockerCookbook::DockerHelpers::Service)
      module Service
        def docker_bin
          '/usr/bin/docker'
        end
        ...
        def systemd_socket_args
          ...
          if systemd_socket_opts
            systemd_socket_opts.each do |systemd_socket_opt|
              opts << "#{systemd_socket_opt}\n"
            end
          end
          opts
        end
        ...
        def docker_daemon_opts
          ...
          if cluster_store_opts
            cluster_store_opts.each do |store_opt|
              opts << "--cluster-store-opt=#{store_opt}"
            end
          end
          ...
          unless default_ip_address_pool.nil?
            opts << "--default-address-pool=#{default_ip_address_pool}"
          end
          ...
          unless disable_legacy_registry.nil?
            opts << "--disable-legacy-registry=#{disable_legacy_registry}"
          end
